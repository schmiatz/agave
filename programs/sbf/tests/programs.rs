...
fn process_transaction_and_record_inner(
    ...
    let executed_units = execution_details.executed_units;
    (result, inner_instructions, log_messages, executed_units)
}
...
fn test_deplete_cost_meter_with_access_violation() {
    ...
    for apply_cost_tracker in [false, true] {
        ...
        let (status, _, _, executed_units) = process_transaction_and_record_inner(&bank, tx);
